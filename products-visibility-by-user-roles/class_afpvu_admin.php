...
if (!class_exists('Addify_Products_Visibility_Admin')) {
	...
	class Addify_Products_Visibility_Admin extends Addify_Products_Visibility {
		...
		public function __construct() {
			...
			add_action('wp_ajax_afpvusearchcategory', array( $this, 'afpvusearchcategory' ));   
		}
		public function afpvusearchcategory() {
		...
		public function afpvu_module_settings() {
			...
			<div class="wrap">
				...
				<h2 class="nav-tab-wrapper">
					<a href="?page=addify-products-visibility&tab=tab_three"
					class="nav-tab <?php echo esc_attr($active_tab) == 'tab_three' ? 'nav-tab-active' : ''; ?>"><?php echo esc_html__('General Settings', 'addify_products_visibility'); ?></a>
					<a href="?page=addify-products-visibility&tab=tab_one"
					class="nav-tab <?php echo esc_attr($active_tab) == 'tab_one' ? 'nav-tab-active' : ''; ?>"><?php echo esc_html__('Global Visibility', 'addify_products_visibility'); ?></a>
					<a href="?page=addify-products-visibility&tab=tab_two"
					class="nav-tab <?php echo esc_attr($active_tab) == 'tab_two' ? 'nav-tab-active' : ''; ?>"><?php echo esc_html__('Visibility By User Roles', 'addify_products_visibility'); ?></a>
				</h2>

				<form method="post" action="options.php" class="addify_pro_visibility">
					<?php
					if ('tab_one' == $active_tab) {
						settings_fields('setting-group-afvisi-1');
						do_settings_sections('addify-products-visibility-1');
					}

					if ('tab_two' == $active_tab) {
						settings_fields('setting-group-afvisi-2');
						do_settings_sections('addify-products-visibility-2');
					}

					if ('tab_three' == $active_tab) {
						settings_fields('setting-group-afvisi-3');
					...
					<?php submit_button(esc_html__('Save Settings', 'addify_products_visibility'), 'primary', 'addify_products_visibility_save_settings'); ?>
				</form>
