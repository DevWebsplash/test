(function( $ ) {
	...
	$(function() {
		...
		jQuery('.afpvu_applied_categories').select2(
		{
			ajax: {
        url: afpvu_php_vars.admin_url, // AJAX URL is predefined in WordPress admin.
        dataType: 'json',
        type: 'POST',
        delay: 20, 
        data: function (params) {
        	return {
                q: params.term,
                action: 'afpvusearchcategory', 
                nonce: afpvu_php_vars.nonce
            };
        },
        processResults: function ( data ) {
        	var options = [];
